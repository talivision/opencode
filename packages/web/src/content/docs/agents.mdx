...
4. Once you are in a child session, use:
   - `session_child_cycle` (default: **\<Leader>+Right**) to cycle to the next child session
   - `session_child_cycle_reverse` (default: **\<Leader>+Left**) to cycle to the previous child session
   - `session_parent` (default: **\<Leader>+Up**) to return to the parent session
