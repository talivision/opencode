...
- `reserved` - Token buffer for compaction. Compaction triggers once the context reaches the model's limit minus this value. Setting it overrides the automatic trigger point entirely.
- `output_floor` - Smallest output window requested for a response, in tokens. Defaults to the smaller of `16384` and the model's output limit, and is never allowed to exceed half of a shared context window. This is a sizing knob only — it never triggers compaction on its own.
- `dynamic_output` - Size the requested output window to the room actually left in the context instead of always reserving the model's full output limit (default: `true`). Set to `false` to restore the previous behavior.

#### Dynamic output window

Models that publish a single combined context limit — which includes every Anthropic model — bill input and output against the same budget. opencode used to reserve the model's full output limit (up to 32k tokens) on every request, so that much context was unavailable even when the reply was one line.

opencode now requests only the room that is actually left, never less than `output_floor`. Long sessions get noticeably more usable context, and compaction on these models triggers slightly later than before. If a request still does not fit, the provider rejects it and opencode compacts and retries as it always has.

Models that publish a separate input limit are unaffected: their input and output budgets are independent, so the full output limit is always requested. Set `dynamic_output` to `false` to opt out.

---

### Goal review

When a goal is active, an independent reviewer agent verifies completion before
the goal can be marked met. You can control how long that reviewer is allowed to
run through the `goal.review` option.
...
- `timeout` - Milliseconds the reviewer may go without any activity before it is
  cancelled (default: `1200000` — 20 minutes). This is an inactivity timer, not a wall clock:
  it resets on streamed text, reasoning, and tool calls, so it only fires on a
  reviewer that has genuinely stalled.
- `max_duration` - Maximum total milliseconds a single review may run, regardless
  of activity (default: `18000000` — 5 hours). This is the backstop against a reviewer that
  stays busy but never converges.

Raise both if you run long-lived goals against large working trees or slow
models. A review that hits either limit is recorded as unverified and the goal
continues working — a timed-out review can never accept a goal as met.

The environment variables `OPENCODE_GOAL_REVIEW_TIMEOUT_MS` and
`OPENCODE_GOAL_REVIEW_MAX_MS` override these for a single run.
