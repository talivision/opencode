...
```json title="tui.json"
{
  ...
  "keybinds": {
    ...
    "goal_minimize": "<leader>z",
    "session_queued_prompts": "none",
...
`permission_cycle` defaults to `shift+tab` and cycles normal → auto → plan → normal.
Entering plan mode selects the `plan` agent; leaving plan mode restores the previously selected agent.
If no plan agent exists, the binding switches between normal and auto instead.
`agent_cycle_reverse` has no default binding, but you can bind it in `tui.json`. An existing configuration that binds it to `shift+tab` keeps that shortcut and disables the permission-cycle default unless `permission_cycle` is also configured.
...
For subagent sessions, the defaults are `session_child_first` = `<leader>down`, `session_child_cycle` = `<leader>right`, `session_child_cycle_reverse` = `<leader>left`, and `session_parent` = `<leader>up`.
