...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const requirePromptableSession = Effect.fn("SessionHttpApi.requirePromptableSession")(function* (
      ...
      if (current.metadata?.goalReviewer !== true) return current
      return yield* new InvalidRequestError({
        message: "Goal reviewer sessions are read-only; their independence is what makes the review meaningful.",
      })
    })
    ...
    const goalSet = Effect.fn("SessionHttpApi.goalSet")(function* (ctx: {
      ...
      if (!objective) return yield* new HttpApiError.BadRequest({})
    ...
    const goalAction = Effect.fn("SessionHttpApi.goalAction")(function* (ctx: {
      params: { sessionID: SessionID }
      payload: typeof GoalActionPayload.Type
    }) {
      yield* requireSession(ctx.params.sessionID)
      if (ctx.payload.action === "edit") {
        const objective = ctx.payload.objective.trim()
        if (!objective) return yield* new HttpApiError.BadRequest({})
        return (
          (yield* goalSvc.edit({
      ...
      if (ctx.payload.action === "pause") return (yield* goalSvc.pause(ctx.params.sessionID)) ?? null
      return (yield* goalSvc.resume(ctx.params.sessionID)) ?? null
    })
      ...
      yield* runState.assertNotBusy(ctx.params.sessionID).pipe(
        Effect.catchTag("SessionBusyError", (error) =>
          Effect.gen(function* () {
            ...
            const targetIndex = messages.findIndex((message) => message.info.id === ctx.params.messageID)
            const target = targetIndex === -1 ? undefined : messages[targetIndex]
            const lastAssistantIndex = messages.findLastIndex((message) => message.info.role === "assistant")
            ...
            // background child's result. Queued is a POSITION in the
            // (time, id)-sorted transcript, not an ID comparison: client
            // clock skew makes IDs lie in both directions.
            const human = target?.parts.some((part) => part.type === "text" && part.synthetic !== true) ?? false
            if (target?.info.role === "user" && human && targetIndex > lastAssistantIndex) return
