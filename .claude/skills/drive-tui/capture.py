...
try:
    ...
    _sys.exit(
        "capture.py needs the 'pyte' terminal emulator to read colours and styles.\n"
        "A local venv (never your global environment) can be provisioned with:\n"
        "  python3 -m venv .claude/skills/drive-tui/.venv\n"
        "  .claude/skills/drive-tui/.venv/bin/pip install -q pyte\n"
        "then run it with that interpreter:\n"
        "  .claude/skills/drive-tui/.venv/bin/python .claude/skills/drive-tui/capture.py <session>\n"
        "Unstyled text is always available via:\n"
        "  tmux -L ${OPENCODE_TMUX_SOCKET:-claude-opencode} capture-pane -p -t <session>"
    )
...
SOCKET = os.environ.get("OPENCODE_TMUX_SOCKET", "claude-opencode")
...
def get_raw_screen(session: str) -> tuple[str, int, int]:
    info = subprocess.run(
        ["tmux", "-L", SOCKET, "display-message", "-t", session, "-p", "#{window_width} #{window_height}"],
        capture_output=True, text=True, check=True,
    )
    cols, rows = map(int, info.stdout.strip().split())
    result = subprocess.run(
        ["tmux", "-L", SOCKET, "capture-pane", "-t", session, "-p", "-e", "-S", "0"],
...
if __name__ == "__main__":
    ...
    if subprocess.run(["tmux", "-L", SOCKET, "has-session", "-t", session], capture_output=True).returncode != 0:
